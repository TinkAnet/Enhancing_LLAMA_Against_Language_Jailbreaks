...
def process_attack_prompt(attack_prompt, attack_id, category):
    ...
    try:
        ...
        if 'content' not in response_data:
            error_message = f"Warning: 'content' key not found in response for entry {attack_id}"
            add_log(error_message, "error")
            stats["error_entries"] += 1
            return {"status": "ERROR", "reason": "Content key not found", "content": None}
            
        if not response_data['content'] or not isinstance(response_data['content'], list):
            error_message = f"Warning: 'content' is empty or not a list for entry {attack_id}"
            add_log(error_message, "error")
            stats["error_entries"] += 1
            return {"status": "ERROR", "reason": "Content empty or not a list", "content": None}
...
def main():
    ...
    output_dir = "implicit_contrast/claude"
